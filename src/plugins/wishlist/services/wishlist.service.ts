...
export class WishlistService {
  constructor(private connection: TransactionalConnection, private productVariantService: ProductVariantService) {}
  ...
  async addItem(ctx: RequestContext, variantId: ID): Promise<WishlistItem[]> {
    ...
    const itemExists = customer.customFields.wishlistItems.some((i) => i.productVariantId === variantId);
    if (itemExists) {
      // Item already exists in wishlist, do not
      // add it again
      return customer.customFields.wishlistItems;
    }
    const wishlistItem = await this.connection.getRepository(ctx, WishlistItem).save(new WishlistItem({ productVariantId: variantId }));
  ...
  private async getCustomerWithWishlistItems(ctx: RequestContext): Promise<Customer> {
    ...
    // use connection.rawConnection if we have no context
    const customer = await this.connection.getRepository(ctx, Customer).findOne({
      where: { user: { id: ctx.activeUserId } },
      // this block is only needed because we set eager to false in the customField
      relations: {
        customFields: {
          wishlistItems: {
            productVariant: true,
